...
const writeInput = (
  ...
  writer
    ...
    .conditionalWriteLine(example !== undefined, `example: "${escapeText(example)}",`)
  ...
  return {
    ...
    initializer: (writer) => {
      ...
      writer
        ...
        .write(",")
        .conditionalWriteLine(examplePayload !== undefined, `examplePayload: ${examplePayload},`)
        .writeLine("})");
...
const writeActionGroup = (project: Project, groupTag: string, actions: Action[]): SourceFile => {
  const file = project.createSourceFile(path.join("src", "actions", `${groupTag}.ts`), undefined, {
...
const writeActionExport = (project: Project, groupTags: string[]): SourceFile => {
  const exportFile = project.createSourceFile(path.join("src", "actions", "index.ts"), undefined, {
    scriptKind: ScriptKind.TS,
  });
...
export const writeActions = (project: Project, actions: Action[]): SourceFile[] => {
